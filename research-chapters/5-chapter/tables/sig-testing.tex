...
\centering
    ...
    \end{subtable} 
    \caption[William's test p-values comparing the difference in performance between each LM and each traditional metric.]{William's test p-values comparing the difference in performance between each LM and each traditional metric. Values that are statistically significant ($\textrm{p-value} < 0.05$), are highlighted in green.}
