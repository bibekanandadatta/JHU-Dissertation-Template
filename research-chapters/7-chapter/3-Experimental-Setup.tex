...
SummEval package to compute the above metrics~\cite{fabbri2021summeval}.
...
Finally, we experiment with a variant of FActScore that measures the utility of information nuggets to the user, rather than the fidelity of the nuggets to the source. We refer to these metrics as Persona Recall and Persona Precision.
...
We use Llama-3.3-70B to generate the LLM-based perturbations (tests~3-5). % We present an analysis of the quality of the generated summaries in~\todo.

We compute the Spearman rank correlation $\rho$~\cite{spearman1904proof} between perturbation level and mean metric score. We additionally report the Monotonicity (M) as the fraction of consecutive level transitions whose sign matches the expected direction. For \emph{stable} tests, a transition is considered correct when the absolute score change is below 5\% of the baseline score.
